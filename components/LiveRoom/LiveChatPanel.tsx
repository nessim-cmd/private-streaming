      ...
      <div ref={listRef} className="flex-1 overflow-y-auto px-3 py-3 space-y-3 bg-black/15">
        {chatMessages.length === 0 ? (
          ...
          chatMessages.map((chatMessage, index) => {
            const senderIdentity = chatMessage.from?.identity;
            const isMine = senderIdentity === localParticipant.identity;
            const senderName = isMine ? "You" : getDisplayName(senderIdentity);
